...
static float *color_gist_gabor(color_image_t *src, const int w, image_list_t *G, float **Features)
{
    fftw_lock();
	...
	// IplImage* gistimg = cvCreateImage(cvSize(5*width, 4*height), IPL_DEPTH_32F, 3);
    // int posh = 0;
	// int posw = 0;
	...
	for(k = 0; k < G->size; k++)
    {
        ...
        for(j = 0; j < height; j++)
        {
            for(i = 0; i < width; i++)
            {
				...
				//Features[j*width + i][0] =  src->c1[j*width+i];
                src->c2[j*width+i] = sqrt(inb2[j*width+i][0]*inb2[j*width+i][0]+inb2[j*width+i][1]*inb2[j*width+i][1])/(width*height);
				//Features[j*width + i][1] =  src->c1[j*width+i];
                src->c3[j*width+i] = sqrt(inb3[j*width+i][0]*inb3[j*width+i][0]+inb3[j*width+i][1]*inb3[j*width+i][1])/(width*height);
				//Features[j*width + i][2] =  src->c1[j*width+i];
				
/* 			 	scal.val[0] = Features[(j*width+i)][0]; //R
				scal.val[1] = Features[(j*width+i)][1]; //G
				scal.val[2] = Features[(j*width+i)][2]; //B
				
				cvSet2D(gistimg,j+posh,i+posw,scal); */
...
/* 	cvNamedWindow("GistScene", CV_WINDOW_AUTOSIZE);
	cvShowImage("GistScene", gistimg);
	cvWaitKey(0);
	cvDestroyWindow("GistScene"); */
    fftwf_destroy_plan(fft1);
    fftwf_destroy_plan(fft2);
...
float *color_gist_scaletab(color_image_t *src, int w, int n_scale, const int *n_orientation, float **Features) 
{
    int i;
    ...
    float *g = color_gist_gabor(img, numberBlocks, G, Features);  
