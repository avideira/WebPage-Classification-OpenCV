...
static float *color_gist_gabor(color_image_t *src, const int w, image_list_t *G, float **Features)
{
	...
	IplImage* gistimg = cvCreateImage(cvSize(5*GIST_SIZE, 4*GIST_SIZE), IPL_DEPTH_32F, 3);
    int posh = 0, posw = 0;
	for(k = 0; k < G->size; k++)
    {
		if(k == 5 || k == 10 || k = 15 )	posw = 0;
        ...
        for(j = 0; j < height; j++)
        {
            for(i = 0; i < width; i++)
            {
				...
				Features[j*width + i][2] =  src->c1[j*width+i];
			 	...
			 	scal.val[0] = Features[(i*lear->width+j)][0]; //R
				scal.val[1] = Features[(i*lear->width+j)][1]; //G
				scal.val[2] = Features[(i*lear->width+j)][2]; //B
				
				cvSet2D(gistimg,i+posw,j+posh,scal);
