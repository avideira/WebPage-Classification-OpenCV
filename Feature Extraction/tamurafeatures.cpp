...
void ExtractTamuraFeat(IplImage *image , double* TamuraVector)
{
	double 	Fdir[16];
	double 	Fcon 		= 0; 
	double 	Fcoar 		= 0;
	double 	numPixels 	= image->imageSize;
	int 	i;
	int 	tamurabins 	= 18;	
...
int sizeLeadDiffValue(int imgheight, int imgwidth, double **A, int k, unsigned int **argm) 
{
	int i,j,x1,x2,y1,y2;
	double tmp, res1, res2, res3, res4;
	double **result;
...
{
	int 		y,x;
	double 		diff = 0.0, norm4 = 0.0, var = 0.0;
	double 		result = 0;
	CvScalar 	s;
	CvScalar 	mean;
    CvScalar 	stddev;
