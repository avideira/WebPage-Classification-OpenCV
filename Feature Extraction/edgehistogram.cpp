...
void EdgeCalculation( unsigned int width, unsigned int height, unsigned short int block_size, double *Local_Edge, IplImage* img )
{
	...
	for( i = 0; i <= (height-block_size); i += block_size )
	{
		...
		{	
			...
			{
			case 	0:		break;
			case 	1:		LongTyp_Local_Edge[local_index*5]++;	break;
			case 	2:		LongTyp_Local_Edge[local_index*5+1]++;	break;
			case 	3:		LongTyp_Local_Edge[local_index*5+2]++;	break;
			case 	4:		LongTyp_Local_Edge[local_index*5+3]++;	break;
			case 	5:		LongTyp_Local_Edge[local_index*5+4]++;	break;
			} 
		}
	}
...
{ 
	int e_index = 0, e, k;
	int threshold = 11;
	double e_h = 0.0 , e_v = 0.0, e_45 = 0.0, e_135 = 0.0, e_m= 0.0, e_max = 0.0;
	double strengths[5];
	...
	int NoEdge						= 	0; 
	int vertical_edge				=	1;
	int horizontal_edge				=	2;
	int non_directional_edge		=	3;
	int diagonal_45_degree_edge		=	4;
	int diagonal_135_degree_edge 	=	5;
